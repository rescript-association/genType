...
let parse_comment
  ...
  let token_stream =
    ...
    in
    Stream.from (fun _token_index -> Some (Odoc_lexer.token input lexbuf))
  in

  match Syntax.parse token_stream with
  | Error.Error error ->
    {Error.result = Error.Error error; warnings = []}
  | Ok ast ->
...
let errors_to_warnings parsed =
  match Error.(parsed.result) with
  | Error.Ok _ ->
    parsed
