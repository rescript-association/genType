module IntMap =
  (Map.Make)(struct
             ...
             end)[@@ocaml.doc " Generate fresh identifiers "]
type typeVarsGen =
  {
  mutable typeNameMap: string IntMap.t ;
  mutable typeNameCounter: int }
let createTypeVarsGen () =
  { typeNameMap = IntMap.empty; typeNameCounter = 0 }
  ...
  | Not_found ->
      (typeVarsGen.typeNameCounter <- (typeVarsGen.typeNameCounter + 1);
       (let name = "T" ^ (string_of_int typeVarsGen.typeNameCounter) in
        typeVarsGen.typeNameMap <-
          (typeVarsGen.typeNameMap |> (IntMap.add id name));
        name))